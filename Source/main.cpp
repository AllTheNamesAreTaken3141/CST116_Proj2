#include <iostream>
#include <windows.h> // Native OS commands. I use it for printing in color but it can probably do some other funky stuff too.
...
// Uses black magic (windows system commands) to print in full color (color_codes.txt has the codes for all 16 colors) with (optional) automatic line breaks. If you end up needing/wanting to put multiple colors on the line then send me a message. I have a function that'll do it without you having to stack this one a bunch of times, but it's a pain to use so I'm not going to add it unless I need to. Anyway, this one takes up to 4 parameters, but you'll probably only ever end up using the first two.
...
string whitespaceInput(string &container)
{
	string input;
